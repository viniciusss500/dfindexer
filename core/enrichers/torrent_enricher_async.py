...
class TorrentEnricherAsync:
    ...
    async def enrich(
        ...
        # Sem Redis, metadata e tracker scraping custam muito (sem cache, cada torrent
        # faz requests externas). Permite desabilitar via env para reduzir latência.
        from app.config import Config
        from cache.redis_client import get_redis_client
        _redis_available = get_redis_client() is not None
        _skip_heavy = not _redis_available and os.getenv('SKIP_ENRICHMENT_WITHOUT_REDIS', 'true').lower() == 'true'
        if _skip_heavy:
            skip_metadata = True
            skip_trackers = True
